<script>
    $(document).ready(function() {
                        ...
                        if (data) {
                            ...
                            setTimeout(() => {
                                $.each(data.translations, function(locale, trans) {
                                    ...
                                    $(`#description_${locale}`).summernote('code',
                                        trans.description || '');
                                    $(`#office_name_${locale}`).val(trans
                                        .office_name);
        ...
        $('#formContactOverview').on('submit', function(e) {
            ...
            showLoading();
            $('#btnSaveContact').prop('disabled', true).html(
            ...
            $.ajax({
                ...
                success: function(res) {
                    hideLoading();
                    $('#btnSaveContact').prop('disabled', false).html(
                        '<i class="ti ti-device-floppy me-1"></i> Save Changes');

                    if (res.status === 'success' || (res.meta && res.meta.status ===
                            'success')) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            text: res.message ||
                                'Contact overview saved successfully!',
                            timer: 2000,
                    ...
                    if (xhr.status === 422 && xhr.responseJSON?.errors) {
                        ...
                        $.each(errors, function(key, messages) {
                            ...
                            if (errorElement.length) {
                                errorElement.text(Array.isArray(messages) ?
                                    messages[0] : messages);
                            }
                        });
